			...
			if (!mitarbeiter.contains(m))
				if (mitarbeiter.size() < 100)
					if (m.getGesch() == 'f' || frauenQuote() >= 40) // entweder Frau, oder Quote passt
						...
						throw new PersonalException("Frauenquote nicht erf�llt!! ");
				else
...
// ----------------------------------------- removes ------------------------------	
	...
	public void removeMitarbeiter(List<Mitarbeiter> mitarbeiterToDelete)
	{
		mitarbeiter.removeAll(mitarbeiterToDelete);
	}
// ----------------------------------------- Sort-Aufrufe ------------------------------	
	...
	{
		Collections.sort(mitarbeiter, new Comparatoren.NameComparator());
	}
	//---------------------------------------- Files ------------------------------------------
